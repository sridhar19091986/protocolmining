 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_PdchNew.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* PdchNew.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* PdchNew.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* PdchNew (application created by MATLAB Compiler)
		* run_PdchNew.sh (shell script that can be run 
	...
	All Platforms:            
		* PdchNew.dll (shared library, file extension 
		  varies by platform)
		* PdchNew.h (library header file)
		* PdchNew.ctf (Component Technology File archive,
	...
	* PdchNew.xml (documentation files)
	* PdchNew.pdb (program database file - if 
	  DEBUG option selected)
	* PdchNew.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* PdchNew.jar (Java class files and compiled M-code)
	...
	* PdchNew.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* PdchNew.xla (Optionally created and placed in
	  G:\protocolmining\PDCH信道配置模型\MatSrc\PdchNew\src directory by the user)  
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
